...
var shortSummary = getGlobalOption('shortSummary', "true") == "true";
...
function GetTraceHeader(problem) {
    ...
    let descContent = "";
    if(problem.trace) {
        descContent += '<a href="'+(shortSummary?"true":"false")+'" style="color:inherit" class="shortSummary"><b>Bug '+(shortSummary?"Short":"Detail")+' Summary</b></a>\n';
        let pfn = problem.file;
        if(pfn.length > 40) pfn = "..."+pfn.slice(-40);
        descContent += 'Sink:  <span class="PathNavNormalSize" style="text-overflow: ellipsis;direction: rtl;font-size:normal !important"><a id="synkLink" href="#tracePoint' + (problem.filteredTrace.length - 1).toString() + '">' + pfn + ' line:' + problem.line + ' col:' + problem.offset + '</a></span>\n';
    ...
    qsa(".shortSummary",
        function(currentValue, currentIndex) {
            currentValue.onclick = function() {
                ...
                setGlobalOption("shortSummary", shortSummary);
                loadReport(lastLoadedProblem);
                return false;
            };
        });           
